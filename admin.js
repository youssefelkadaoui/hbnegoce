...
function authMessage(text = '', success = false) {
  ...
  el.classList.toggle('error', !success && Boolean(text));
}
...
function displayProducts() {
  ...
  if (!products.length) {
    list.innerHTML = '<div class="empty">لا توجد منتجات في Firebase بعد. يمكنك إضافة منتج أو استيراد المنتجات الحالية.</div>';
    return;
  }
  ...
  list.innerHTML = products.map((product) => `
    <article class="admin-product">
      ...
      <div>
        ...
        <p>${escapeHtml(product.brand)} · ${escapeHtml(String(product.price))} درهم</p>
      </div>
      <div class="item-actions">
        <button class="icon-btn" data-edit="${escapeHtml(String(product.id))}" title="تعديل"><i class="fas fa-pen"></i></button>
        <button class="icon-btn delete" data-delete="${escapeHtml(String(product.id))}" title="حذف"><i class="fas fa-trash"></i></button>
      </div>
    </article>
  `).join('');
}
...
function startEdit(id) {
  ...
  $('name').value = p.name || '';
  $('brand').value = p.brand || '';
  $('type').value = p.type || 'كلاسيك';
  $('price').value = p.price ?? '';
  $('oldPrice').value = p.oldPrice ?? '';
  $('desc').value = p.desc || '';
  $('specs').value = (p.specs || []).join('\n');
  $('currentImages').innerHTML = (p.images || [p.image]).filter(Boolean).map((url) => `<img src="${escapeHtml(url)}" alt="صورة المنتج" />`).join('');
...
async function uploadImages(files, id) {
  ...
  for (const file of files) {
    ...
    const target = storageRef(storage, `products/${id}/${Date.now()}-${safeName}`);
    await withTimeout(uploadBytes(target, file), 'upload');
    urls.push(await withTimeout(getDownloadURL(target), 'upload'));
  }
  return urls;
}
...
$('loginForm').addEventListener('submit', async (event) => {
  ...
  try {
    await withTimeout(signInWithEmailAndPassword(auth, $('loginEmail').value.trim(), $('loginPassword').value), 'login');
    authMessage('تم تسجيل الدخول بنجاح.', true);
  } catch (error) {
    console.error('Firebase login failed:', error);
    authMessage(loginErrorMessage(error));
  } finally {
    button.disabled = false;
    button.innerHTML = '<i class="fas fa-right-to-bracket"></i> تسجيل الدخول';
  }
});
...
productForm.addEventListener('submit', async (event) => {
  ...
  const id = $('productId').value || String(Date.now());
  ...
  try {
    ...
    const product = {
      ...
      images,
      variants: images.map((image, index) => ({ index: index + 1, image })),
      updatedAt: Date.now()
    };

    await withTimeout(set(ref(database, `products/${id}`), product), 'database');
    clearForm();
    message('تم حفظ المنتج وسيظهر في المتجر تلقائيًا.');
  } catch (error) {
    console.error('Product save failed:', error);
    message(saveErrorMessage(error), true);
  } finally {
    saveBtn.disabled = false;
    saveBtn.innerHTML = '<i class="fas fa-floppy-disk"></i> حفظ المنتج';
  }
});
...
$('productsList').addEventListener('click', async (event) => {
  ...
  if (!del || !confirm('هل تريد حذف هذا المنتج؟')) return;

  try {
    await remove(ref(database, `products/${del.dataset.delete}`));
    message('تم حذف المنتج.');
  } catch {
    message('تعذر الحذف. تحقق من قواعد قاعدة البيانات.', true);
  }
});

$('importDefaultsBtn').addEventListener('click', async () => {
  const defaults = window.hbProducts || [];
  if (!defaults.length || !confirm(`سيتم نشر ${defaults.length} منتجات حالية في Firebase. متابعة؟`)) return;

  try {
    await Promise.all(defaults.map((p) => set(ref(database, `products/${p.id}`), { ...p, updatedAt: Date.now() })));
    message('تم استيراد المنتجات الحالية بنجاح.');
  } catch {
    message('تعذر الاستيراد. تحقق من قواعد قاعدة البيانات.', true);
  }
});
